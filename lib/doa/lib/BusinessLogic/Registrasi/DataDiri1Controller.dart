...
import 'package:eformplugin/doa/lib/response_model/device_properties.dart';
import 'package:eformplugin/doa/lib/response_model/user_check_model.dart';
import 'package:eformplugin/doa/lib/service/services.dart';
import 'package:eformplugin/doa/lib/src/components/Global.dart';
import 'package:eformplugin/doa/lib/src/components/alert-dialog-new-wrap.dart';
...
class DataDiri1Controller extends GetxController {
  ...
  var initialDate = DateTime(
      DateTime.now().year - 17, DateTime.now().month, DateTime.now().day);
  ...
  Future<void> submit() async {
    ...
    try {
      ...
      var _res = await Services()
          .POST(urlCekNIK, 'Api Check Account', body: await _body);
      ...
      assert(Global().compareableJwtBody(
          responseBody: userCheckModel?.toJson().values.toList(),
          jwtBody: _jwtBody));
      update();
      if ((userCheckModel?.errorCode == null) ||
          userCheckModel!.errorCode!.isEmpty) {
        ...
        prefs!.setString(
            'tanggalLahir', DateFormat('yyyy-MM-dd').format(newDate));
  ...
  getStringValuesSF() async {
    ...
    if (prefs!.getString('nik') != '' && prefs!.get("nik") != null) {
      ...
      if (Get.find<OtpController>().errorcodeCreateAccount != '') {
        ...
        Get.find<OtpController>().errorcodeCreateAccount = '';
          ...
          Get.find<OtpController>().headerErrorCode == '9001') {
    ...
    // print('ini adalah produk : $product');

    if (product == 'Taplus Muda Digital') {
      firstDate = DateTime(
          DateTime.now().year - 35, DateTime.now().month, DateTime.now().day);
    } else {
      firstDate = DateTime(
  ...
  Future<bool> willPop(SharedPreferences _prefs) async {
    if (_prefs.getString('produk') == 'Taplus Digital') {
      Get.off(() => CardTypeScreen(), transition: Transition.rightToLeft);
    } else if (_prefs.getString('produk') == 'Taplus Muda Digital') {
      Get.off(() => CardTypeMuda(), transition: Transition.rightToLeft);
    } else if (_prefs.getString('produk') == 'Taplus Bisnis Digital') {
