...
import 'package:eformplugin/doa/lib/response_model/create_account_model.dart';
import 'package:eformplugin/doa/lib/response_model/data_diri1_model_pref.dart';
import 'package:eformplugin/doa/lib/response_model/send_otp_model_response.dart';
import 'package:eformplugin/doa/lib/service/services.dart';
import 'package:eformplugin/doa/lib/src/models/Status.dart';
...
class OtpController extends GetxController {
  ...
  Future<Map<String, dynamic>> get _body async {
    ...
    return {
      ...
      "othersOpenAccReason":
          othersOpenAccReason, //tujuan pembukaan dana lainnya
      //dari halaman pilih pekerjaan,tujuan pembukaan rekening
...
//ENHANCE SERVICE SENDOTP
  ...
  Future<void> submitSendOtp() async {
    ...
    try {
      ...
      var _res = await Services()
          .POST(urlSendOtpAndCreateAccount, 'Api Send Otp', body: body);
      ...
      if (_res?.statusCode == 200) {
        ...
        var _jwtBody =
            (_res?.data?.body as Map<String, dynamic>).values.toList();

        assert(Global().compareableJwtBody(
            responseBody: sendOtpModelResponse?.toJson().values.toList(),
            jwtBody: _jwtBody));

        if ((sendOtpModelResponse?.errorCode == null) ||
            sendOtpModelResponse!.errorCode!.isEmpty) {
          CustomLoading().dismissLoading();
          Get.offNamed(Routes().otp);
        } else if (sendOtpModelResponse!.errorCode == '9001') {
          headerErrorCode = '9001';
  ...
  void countdown(int _timer) {
    periodicTimer = RestartableTimer(
...
//ENHANCE SERVICE CREATE ACCOUNT
  ...
  Future<void> createAccountDio() async {
    ...
    try {
      ...
      var _res = await Services().POST(
          urlSendOtpAndCreateAccount, 'Api Create Account',
      ...
      if (_res?.statusCode == 200) {
        createAccountModelResponse =
            CreateAccountModelResponse.fromJson(_res?.data?.body);

        if ((_res?.toJson()['data']['body']['errorCode']) != '') {
          CustomLoading().dismissLoading();
          errorcodeCreateAccount =
              createAccountModelResponse?.errorCode as String;
  ...
  getData() async {
    ...
    compressedSelfiePhotoSize =
        prefs!.getString('compressedSelfiePhotoSize') ?? '';

    originalIdSelfiePhotoSize =
        prefs!.getString('originalIdSelfiePhotoSize') ?? '';
    compressedIdSelfiePhotoSize =
        prefs!.getString('compressedIdSelfiePhotoSize') ?? '';

    originalIdPhotoSize = prefs!.getString('originalIdPhotoSize') ?? '';
    compressedIdPhotoSize = prefs!.getString('compressedIdPhotoSize') ?? '';

    originalSignaturePhotoSize =
        prefs!.getString('originalSignaturePhotoSize') ?? '';
    compressedSignaturePhotoSize =
        prefs!.getString('compressedSignaturePhotoSize') ?? '';

    idNumber = prefs!.getString('nik') ?? '';
    // print(idNumber);

    phoneNumber = prefs!.getString('dialCodeNegara')! +
        prefs!.getString('nomorHandphone')!;
    ...
    landline =
        '${prefs!.getString('prefixnoTelpRumah')}${prefs!.getString('noTelpRumah')}';
  ...
  Future<void> savePhotoDio() async {
    ...
    var _resSelfie = await Services()
        .POST(urlSavePhoto, 'Api Save Photo', body: bodySavePhotoSelfie);
    ...
    var _resKTP = await Services()
        .POST(urlSavePhoto, 'Api Save Photo', body: bodySavePhotoKtp);
    // print("Ini Adalah Status Code Dari Save Photo KTP" +
    //     _resKTP!.statusCode.toString());
    await Future.delayed(Duration(seconds: 3));
    var bodySavePhotoTTD = await _bodySavePhotoTTD;
    var _resTTD = await Services()
        .POST(urlSavePhoto, 'Api Save Photo', body: bodySavePhotoTTD);
