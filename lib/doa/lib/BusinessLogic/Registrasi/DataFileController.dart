...
import 'package:eformplugin/doa/lib/src/components/Global.dart';
...
import '../../src/components/zoloz_sdk.dart';
...
class DataFileController extends GetxController {
  ...
  Rx<Checkfaceresult?> checkfaceresult = Checkfaceresult().obs;
  void updateChekcFace(Checkfaceresult checkfaceresultParam) {
    checkfaceresult.value = checkfaceresultParam;
  }
  ...
  get _bodycekLivenessType async {
  ...
  Future<void> cekLivenessType() async {
    var response = await Services().POST(
        urlcekLivenessType, 'Cek Liveness Type',
        body: await _bodycekLivenessType);

    livenessType = response?.data?.body['livenessType'];
    print('ini adalah : ${livenessType}');
    return;
  }

  final GlobalKey<ScaffoldState> scaffoldKey = GlobalKey<ScaffoldState>();
  get bodySavePhotoSelfie async {
    final devicePropertiesPlugin = DeviceInfo();
    var device = await devicePropertiesPlugin.getDeviceInfo();
    return {
      'type': 'liveness',
...
//CHECK ERROR DENGAN SERVICE BARU
  checkError() {
    if (Get.find<OtpController>().errorcodeCreateAccount != '') {
      var errorCode = Get.find<OtpController>().errorcodeCreateAccount;
      ...
      Get.find<OtpController>().errorcodeCreateAccount = '';
    }
  }
  ...
  get cobainit async {
    ...
    int randomNumberBizId = Random().nextInt(900000) + 100000;
    var refNumRandom = DateFormat('yyyyMMddhhmmssSSS').format(DateTime.now()) +
        randomNumber.toString();
    var bizIdRandom = DateFormat('yyyyMMddhhmmss').format(DateTime.now()) +
        randomNumberBizId.toString();
    return {
      'bizId': bizIdRandom,
      // 'docType': "00000001003",
      'metaInfo': _metaInfo,
  ...
  Future<void> startZolozLiveness({bool isOnlyLiveness = false}) async {
    int randomNumber = Random().nextInt(900) + 100;
    int randomNumberBizId = Random().nextInt(900000) + 100000;
    var refNumRandom = DateFormat('yyyyMMddhhmmssSSS').format(DateTime.now()) +
        randomNumber.toString();
    ...
    var _faceInitPayload = FaceInitPayload(
        ...
        channel: 'EFORM-MOBILE',
        refNum: refNumRandom);
    print('ini face init payload : ${jsonEncode(_faceInitPayload.toJson())}');

    var response = await ZOLOZ_SDK_RESULT(await cobainit,
        faceInitPayload: _faceInitPayload, isOnlyLiveness: isOnlyLiveness);
    CustomLoading().dismissLoading();

    // print(
    //     'Ini Adalah cekfaceresult di controller : ${checkfaceresult.toJson()}');
    // print('ini response zoloz : ${response?.toJson()}');
    // print('ini response zoloz : ${response?.faceAttack}');
  }
  ...
  //           var body = await bodySavePhotoSelfie;
  //           await Future.delayed(Duration(seconds: 3));

  //           var _resSelfie = await Services()
  //               .POST(urlSavePhoto, 'Api Save Photo', body: body);
  ...
  //         //   return;
  //         // }
  //         dev.log('Ini Body ${bodySavePhotoSelfie}');
  ...
  //         children: [
  //           Center(
  //             child: SvgPicture.asset('/images/icons/bell_icon.svg'),
  //           ),
  //           SizedBox(
  //             height: 8,
  //           ),
