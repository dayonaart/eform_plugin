...
import 'package:eformplugin/doa/lib/response_model/create_mbank_model_response/create_mbank_model_response.dart';
...
''';
  ...
  Future<void> submit() async {
    ...
    var _resCekMbank = await Services()
        .POST(urlcheckCIFMbank, 'Api Check Mbank', body: _bodyCekMbank);
    // if (kDebugMode) {
    ...
    if (_resCekMbank!.statusCode == 500) {
      ...
      if (data == '9001') {
        var _resCreateMbank = await Services()
            .POST(urlcheckCIFMbank, 'Api Create Mbank', body: _bodyCreateMbank);
  ...
  Future<void> cekMbank(
    ...
    try {
      ...
      errorCode = await cekMbankService().cekMbank(
          username: username, email: email, phone: phonenum, cifnum: cifnum);
