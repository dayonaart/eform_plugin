...
''';
  ...
  Future<void> submit() async {
    ...
    var _res = await Services()
        .POST(urlcheckCIFMbank, 'Api Check Cif Mbank', body: _body);
  ...
  cekMif(
    ...
    try {
      // emit(CekmifLoading());
      // print(state);
      errorCode = await CheckMIFService()
  ...
  checkProductType(String productType) {
    // print(productType);
    if (productType == 'Taplus Muda Digital') {
      minimumDeposit = '100.000,-';
      update();
      // setState(() {
      // });
    } else if (productType == 'Taplus Digital') {
      minimumDeposit = '250.000,-';
      update();

      // setState(() {
      // });
    } else if (productType == 'Taplus Bisnis Digital') {
    ...
    depositDeadline = DateFormat('dd MMMM yyyy', 'id_ID')
  ...
  Future<void> generatePDF() async {
    ...
    final icon = pw.MemoryImage(
      (await rootBundle.load('assets/images/simpanan-logo.png'))
          .buffer
          .asUint8List(),
    );
          ...
          return pw.Column(
              ...
              children: [
                pw.Align(
                    ...
                    child: pw.Container(
                        child: pw.Image(icon), width: 90, height: 30)),
                pw.SizedBox(height: 60),
                pw.Text('Pembukaan Rekening Anda sudah berhasil',
                ...
                pw.RichText(
                  text:
                      pw.TextSpan(style: pw.TextStyle(fontSize: 14), children: [
                    pw.TextSpan(
                        text:
                ...
                pw.RichText(
                  text:
                    ...
                    pw.TextSpan(
                        text:
                            'Anda dapat melakukan pengambilan kartu debit fisik melalui '),
                    ...
                    pw.TextSpan(
                        text:
                            'Jangan lupa siapkan e-KTP dan bukti pembukaan rekening Anda.'),
                  ]),
                ),
  ...
  getStringValuesSF(
      String? accountNum, String errorCode, String? Cardnum) async {
    ...
    if (Get.find<CreatePinController>().getCardModelResponse?.accountNum ==
        null) {
      valueAck.add('');
    } else {
