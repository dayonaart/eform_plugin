...
import 'package:eformplugin/doa/lib/src/components/dropdown.dart';
...
class DataDiri2Controller extends GetxController {
    ...
    switch (name) {
      ...
      case "city":
        ...
        List listkotaFilter = cityList
            .where((x) =>
                ...
                provinceController.text.toLowerCase())
            .toList();
        final result = await showDialog(
            context: context,
            builder: (_) => DropDownFormField(
      ...
      case "district":
        ...
        List listkecamatanFilter = subDistrictList
            .where((x) =>
                x['idkota'].toLowerCase() == cityController.text.toLowerCase())
            .toList();
        final result = await showDialog(
            context: context,
            builder: (_) => DropDownFormField(
      ...
      case "postalCode":
        ...
        List listkodePosFilter = postalCodeList
            .where((x) =>
                ...
                subDistrictController.text.toLowerCase())
            .toList();
        final result = await showDialog(
          context: context,
          builder: (_) => DropDownFormField(
  ...
  Future<void> readProvinceJson() async {
    final String response =
        await rootBundle.loadString('$_jsonObject/provinsi.json');
  ...
  Future<void> readCityJson() async {
    final String response =
        await rootBundle.loadString('$_jsonObject/kota.json');
    final data = await json.decode(response);
  ...
  Future<void> readSubDistrictJson() async {
    final String response =
        await rootBundle.loadString('$_jsonObject/kecamatan.json');
    final data = await json.decode(response);
  ...
  Future<void> readPostalCodeJson() async {
    final String response =
        await rootBundle.loadString('$_jsonObject/kodepos.json');
    final data = await json.decode(response);
  ...
  Future<void> readCountryJson() async {
    final String response =
        await rootBundle.loadString('$_jsonObject/codePhone.json');
    final data = await json.decode(response);
  ...
  getStringValuesSF() async {
    ...
    Future.delayed(Duration(seconds: 1), () async {
      ...
      if (prefs!.getString('indexPosisi') == '2') {
        ...
        overseasAddressController.text =
            prefs!.getString('alamatDomisili') ?? "";
      }
    });
  ...
  addStringToSF() async {
    ...
    if (landlineNumberController.text == '' ||
        landlineNumberController.text.isEmpty) {
