...
class WorkDetailController extends GetxController {
  ...
  Future setSharedPrefWorkDetail(String jobDetail, jobPlace, officePhoneNumber,
      officeAddress, officePostalCode) async {
    ...
    if (jobDetail.text.length >= 5 && jobDetail.text.length <= 40) {
      ...
      if (jobPlace.text.length >= 5 && jobPlace.text.length <= 60) {
        ...
        if (officeAddress.text.length >= 5 && officeAddress.text.length <= 40) {
          print("office here");
          if (officePostalCode.text.length >= 5 &&
              officePostalCode.text.length <= 5) {
            setSharedPrefWorkDetail(
                jobDetail.text,
                jobPlace.text,
