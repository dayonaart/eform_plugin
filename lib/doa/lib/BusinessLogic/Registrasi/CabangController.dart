...
class CabangController extends GetxController {
      ...
      case "city":
        if (province.text.isNotEmpty) {
          List cityListFilter = cityList
              .where((x) => x['idprovinsi']
                  .toLowerCase()
                  .contains(province.text.toLowerCase()))
              .toList();
      ...
      case "office":
        if (city.text.isNotEmpty) {
          ...
          List officeListFilter = officeList
              .where((x) =>
                  x['kota'].toLowerCase().contains(city.text.toLowerCase()))
              .toList();
          ...
          if (response != null) {
            ...
            List biCodeListFilter = biCodeList
                .where((x) => x['SANDI_BNI'].contains(response["kodeoutlet"]))
                .toList();
            update();
  ...
  Future getStringValuesSF() async {
    ...
    if (selectedProvince.isNotEmpty) {
      index = provinceList
          .indexWhere((element) => element["id"] == selectedProvince);
      province.text = provinceList[index]['provinsi'];
      index =
          cityList.indexWhere((element) => element["idkota"] == selectedCity);
      city.text = cityList[index]['kota'];
      index = officeList
          .indexWhere((element) => element["kodeoutlet"] == selectedOffice);
