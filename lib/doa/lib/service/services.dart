...
class Services {
  ...
  Future<WrapResponse?> POST(
    ...
    try {
      ...
      var _execute = Dio(_baseDioOption(url: customBaseUrl ?? endpoint)).post(
          endpoint,
      ...
      if (e.type == DioErrorType.response) {
        ...
        ERROR_SNACK_BAR(
            title: "Perhatian", message: "Pastikan Anda terhubung ke Internet");
  ...
  Future<JwtResult?> _tryDecodeJwt(dio.Response<dynamic> _res) async {
    try {
      ...
      return await _jwtsonPlugin.decodeJwtToken(
          key: Global()
              .generateSecretJwt(timepstamp: _res.data['header']['timestamp']),
  ...
  Future<WrapResponse?> GET(String endpoint, String step,
    ...
    var _net = await checkNetwork();
    try {
      var _execute = Dio(_baseDioOption(url: customBaseUrl ?? endpoint)).get(
          endpoint,
      ...
      if (e.type == DioErrorType.response) {
        showErrorSnackbar
            ? ERROR_DIALOG(
                title: '${e.response?.statusCode}',
                message: "service sedang maintenance")
            : null;
        return WrapResponse(
            message: e.response?.statusMessage ?? e.message,
            statusCode: e.response?.statusCode ?? 0,
