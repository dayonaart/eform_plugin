...
class CheckMIFService {
  Future<String> cekCIF(
      {required cifNum, required phone, required email}) async {
    var url = Uri.parse(urlcheckCIFMbank);
    Xml2Json xml2Json = Xml2Json();

    var headers =
        Get.find<DataController>().headerSoap(step: 'Api Check Cif Mbank');
    var body = '''
...
//Data dinamis
    ...
    //         .toString());
    var data = jsonDecode(jsondatatoGData)['soapenv\$Envelope']['soapenv\$Body']
                ['soapenv\$Fault']['detail']
            ['ne\$inquiryAllParamFault1_appFault']['errorNum']['\$t']
        .toString();
