...
class PemilihanCabangLuarPage extends StatefulWidget {
  ...
  @override
  State<PemilihanCabangLuarPage> createState() =>
      _PemilihanCabangLuarPageState();
}
      ...
      child: MediaQuery.removePadding(
        ...
        child: Scaffold(
            ...
            headerTextDetail: RichText(
              text: TextSpan(
                ...
                children: [
                  ...
                  TextSpan(
                      text: "terdaftar di BNI Kantor Cabang Jakarta Pusat.",
                      style: semibold14),
                ],
              ),
            ),
          ...
          floatingActionButton: ButtonCostum(
            ...
            ontap: () {
              setState(() {
                List biCodeListFilter = _biCodeList
                    .where((x) => x['SANDI_BNI'].contains("259"))
                    .toList();
                List officeListFilter = _officeList
                    .where((x) => x['kodeoutlet'].contains("259"))
                    .toList();
                Future saveSharedPref() async {
                  final preferences = dataController.prefs;
                  preferences.setString(
                      'sandiBI', biCodeListFilter[0]["SANDI_BI"].toString());
                  preferences.setString(
                      'kotaBI', biCodeListFilter[0]["KOTA"].toString());
                  preferences.setString('namaOutlet',
                      officeListFilter[0]["namaoutlet"].toString());
                  preferences.setString('kodeOutlet',
                      officeListFilter[0]["kodeoutlet"].toString());
                  preferences.setString(
                      'alamatOutlet', officeListFilter[0]["alamat"].toString());
